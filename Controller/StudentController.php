...
class StudentController
{
    ...
    function GetStateLocalStudent($localStudents, $student)
    {
        ...
        for ($i = 0; $i < count($localStudents); $i++) {
            $customFields = isset($student[StudentDAO::$CUSTOM_FIELDS_COLUMN]) ? json_encode($student[StudentDAO::$CUSTOM_FIELDS_COLUMN]) : null;
            $localCustomFields = $localStudents[$i][StudentDAO::$CUSTOM_FIELDS_COLUMN] == '[]' ? null : $localStudents[$i][StudentDAO::$CUSTOM_FIELDS_COLUMN];
            $studentRS[SyncDAO::$STATE_COLUMN] = $student[StudentDAO::$ID_COLUMN] == $localStudents[$i][StudentDAO::$ID_COLUMN] && $student[StudentDAO::$FIRST_NAME_COLUMN] == $localStudents[$i][StudentDAO::$FIRST_NAME_COLUMN] && $student[StudentDAO::$LAST_NAME_COLUMN] == $localStudents[$i][StudentDAO::$LAST_NAME_COLUMN] && $student[StudentDAO::$EMAIL_COLUMN] == $localStudents[$i][StudentDAO::$EMAIL_COLUMN] && $customFields == $localCustomFields ? SyncDAO::$STATE_OK_COLUMN : $studentRS[SyncDAO::$STATE_COLUMN];
            $studentRS[SyncDAO::$STATE_COLUMN] = $student[StudentDAO::$ID_COLUMN] == $localStudents[$i][StudentDAO::$ID_COLUMN] && ($student[StudentDAO::$FIRST_NAME_COLUMN] != $localStudents[$i][StudentDAO::$FIRST_NAME_COLUMN] || $student[StudentDAO::$LAST_NAME_COLUMN] != $localStudents[$i][StudentDAO::$LAST_NAME_COLUMN] || $student[StudentDAO::$EMAIL_COLUMN] != $localStudents[$i][StudentDAO::$EMAIL_COLUMN] || $customFields != $localCustomFields) ? SyncDAO::$STATE_UPDATE_COLUMN : $studentRS[SyncDAO::$STATE_COLUMN];
        }
